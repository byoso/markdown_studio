...
def render_html(markdown_text: str, css_href: str | None = None) -> str:
    return _wrap_document(render_body_html(markdown_text), css_href=css_href)
...
def render_pages_html(
    ...
    return _wrap_document("\n".join(pages), css_href=css_href)


def render_shell_html(css_href: str | None = None) -> str:
    ...
    return _wrap_document(
        f'<div id="md-content"></div>{spacer_style}', css_href=css_href
    )


def _wrap_document(body: str, css_href: str | None = None) -> str:
    css_link = f'<link rel="stylesheet" href="{css_href}">' if css_href else ""
    image_style = "<style>img { max-width: 100%; height: auto; }</style>"
    return f"""<!DOCTYPE html>
<html>
<head><meta charset="utf-8">{css_link}{image_style}</head>
<body>
{body}
</body>
</html>"""
